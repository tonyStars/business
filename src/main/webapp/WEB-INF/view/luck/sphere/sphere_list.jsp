...
<script>
    layui.use(['form', 'table','laydate'], function () {
        ...
        var renderTable = function (){
            table.render({
                elem: '#currentTableId',
                url: path + "/luck/sphere/search.do",
                cols: [[
                    ...
                    {field: 'redNumber6', width: 120, title: '红号码6'},
        ...
        $(".data-add-btn").on("click", function () {
            layer.open({
                ...
                maxmin: false, // 放大缩小按钮
                content: path + "/luck/sphere/toAdd.do",
        ...
        $(".data-luck-btn").on("click", function () {
            layer.open({
                ...
                fixed: false, // 鼠标滚动时，层是否固定在可视区域
                maxmin: false, // 放大缩小按钮
                content: path + "/luck/sphere/toLuck.do",
                success: function(layero, index){ // 层弹出成功后回调参数：当前层DOM&当前层索引
                    //键盘按键监听
                    $(document).on("keydown", function(e){
                        if (e.keyCode == 13){ // 禁用回车事件 以免层重复弹出
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                layer.open({
                    ...
                    fixed: false, // 鼠标滚动时，层是否固定在可视区域
                    maxmin: false, // 放大缩小按钮
                    content: path + "/luck/sphere/toEdit.do?id=" + data.id,
                    success: function(layero, index){ // 层弹出成功后回调参数：当前层DOM&当前层索引
                        //键盘按键监听
                        $(document).on("keydown", function(e){
                            if (e.keyCode == 13){ // 禁用回车事件 以免层重复弹出
