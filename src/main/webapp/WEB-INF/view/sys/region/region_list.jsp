...
<script>
    layui.use(['form', 'table'], function () {
        ...
        var renderTable = function (){
            table.render({
                elem: '#currentTableId',
                url: path + "/sys/region/search.do",
                cols: [[
                    {type: "checkbox", width: 50, fixed: "left"},
                    {field: 'code', width: 120, title: '区域编码'},
                    {field: 'name', minWidth: 200, title: '区域名称'},
                    {field: 'parentName', minWidth: 200, title: '父地区名称'},
                    {
                        field: 'regionType', width: 100, align: 'center', templet: function (d) {
                            if (d.regionType == 0) {
                                return '国家';
                            } else if (d.regionType == 1) {
                                return '省';
                            } else if (d.regionType == 2){
                                return '市';
                            } else if (d.regionType == 3){
                                return '区';
                            } else{
                                return '直辖市';
                            }
        ...
        $(".data-add-btn").on("click", function () {
            layer.open({
                ...
                maxmin: false, // 放大缩小按钮
                content: path + "/sys/region/toAdd.do",
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                ...
                layer.open({
                    ...
                    fixed: false, // 鼠标滚动时，层是否固定在可视区域
                    maxmin: false, // 放大缩小按钮
                    content: path + "/sys/region/toEdit.do?regionId=" + data.regionId,
                    success: function(layero, index){ // 层弹出成功后回调参数：当前层DOM&当前层索引
                        //键盘按键监听
                        $(document).on("keydown", function(e){
                            if (e.keyCode == 13){ // 禁用回车事件 以免层重复弹出
    ...
    function reloadTable(){
        layui.use('table', function(){
            ...
            var regionType = $.trim($('#regionType').val());
