...
<div class="layuimini-container">
    <div class="layuimini-main">
                ...
                <form class="layui-form layui-form-pane">
                    <div class="layui-form-item">
                        ...
                        <div class="layui-inline">
                            ...
                            <div class="layui-input-inline">
                                <select id="companyLevel" name="companyLevel"></select>
                            </div>
                        </div>
                        <div class="layui-inline">
        ...
        <div class="layui-btn-group">
            ...
            <button class="layui-btn layui-icon data-print-btn">&#xe60d;打印</button>
        </div>
...
<script>
    ...
    layui.use(['form', 'table'], function () {
        ...
        TYPE_CODE = initDicSelect($("#typeCode"),"公司类型");
        COMPANY_LEVEL = initDicSelect($("#companyLevel"),"公司级别");
        var renderTable = function (){
            table.render({
                elem: '#currentTableId',
                url: path + "/sys/company/search.do",
                cols: [[
                    {type: "checkbox", width: 50, fixed: "left"},
                    {field: 'companyCode', width: 120, title: '公司编码'},
                    {field: 'companyName', minWidth: 200, title: '公司全称'},
                    {field: 'parentName', minWidth: 200, title: '父公司名称'},
                    {
                        field: 'typeCode', width: 100, align: 'center', templet: function (d) {
                            for(var i = 0; i < TYPE_CODE.length; i++){
                                if(TYPE_CODE[i].value == d.typeCode){
                                    return TYPE_CODE[i].itemName;
                                }
                            }
                        }, title: '公司类型'
                    },
                    {
                        field: 'companyLevel', width: 100, align: 'center', templet: function (d) {
                            for(var i = 0; i < COMPANY_LEVEL.length; i++){
                                if(COMPANY_LEVEL[i].value == d.companyLevel){
                                    return COMPANY_LEVEL[i].itemName;
                                }
                            }
                        }, title: '公司级别'
                    },
        ...
        form.on('submit(data-search-btn)', function (data) {
            ...
            table.reload('testReload', {
                ...
                , where: {
                    ...
                    companyLevel: result.companyLevel,
        ...
        $(".data-add-btn").on("click", function () {
            layer.open({
                ...
                maxmin: false, // 放大缩小按钮
                content: path + "/sys/company/toAdd.do",
        ...
        $(".data-print-btn").on("click", function () {
            ...
            commonPrint(Public.print.SYSCOMPANY,"",ids);
        });
        /**监听表格复选框选择*/
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                layer.open({
                    ...
                    fixed: false, // 鼠标滚动时，层是否固定在可视区域
                    maxmin: false, // 放大缩小按钮
                    content: path + "/sys/company/toEdit.do?companyId=" + data.companyId,
                    success: function(layero, index){ // 层弹出成功后回调参数：当前层DOM&当前层索引
                        //键盘按键监听
                        $(document).on("keydown", function(e){
                            if (e.keyCode == 13){ // 禁用回车事件 以免层重复弹出
    ...
    function reloadTable(){
        layui.use('table', function(){
            ...
            var companyLevel = $.trim($('#companyLevel').val());
